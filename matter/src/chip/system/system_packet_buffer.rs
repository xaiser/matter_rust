...
const ZEROED_PACKETBUFFER: BufferPoolElement = BufferPoolElement {
    block: [0; PacketBuffer::KBLOCK_SIZE as usize],
};

static mut S_BUFFER_POLL: [BufferPoolElement; CHIP_SYSTEM_CONFIG_PACKETBUFFER_POOL_SIZE as usize] =
...
impl PacketBuffer {
    pub const KSTRUCTURESIZE: u16 =
        chip_system_align_size!(mem::size_of::<PacketBuffer>(), 4) as u16;
    pub const KMAX_SIZE_WITHOUT_RESERVE: u32 = CHIP_SYSTEM_CONFIG_PACKETBUFFER_CAPACITY_MAX;
    pub const KBLOCK_SIZE: u16 =
        PacketBuffer::KSTRUCTURESIZE + PacketBuffer::KMAX_SIZE_WITHOUT_RESERVE as u16;
    ...
    pub const KMAX_SIZE: u32 =
        Self::KMAX_SIZE_WITHOUT_RESERVE - Self::KDEFAULT_HEADER_RESERVE as u32;
    pub const KLARGE_BUFFER_MAX_SIZE_WITHOUT_RESERVE: u32 =
    ...
    pub fn reserve_start_const(&self) -> *const u8 {
        ...
        return unsafe {
            start
                .add(PacketBuffer::KSTRUCTURESIZE as usize)
                .cast::<u8>()
        };
    }
    ...
    fn clone(&self) -> Self {
        ...
        unsafe {
            while original.is_null() == false {
                ...
                if original_data_size + usize::from(original_reserved_size)
                    > (PacketBuffer::KMAX_ALLOC_SIZE as usize)
                {
                    ...
                    if usize::from(original_reserved_size) + ((*original).data_len() as usize)
                        > (PacketBuffer::KMAX_ALLOC_SIZE as usize)
                    {
                    ...
                    original_data_size = (PacketBuffer::KMAX_ALLOC_SIZE as usize)
                        - (original_reserved_size as usize);
                }

                if let Some(clone) = PacketBufferHandle::new(
                    original_data_size.try_into().unwrap(),
                    original_reserved_size,
                ) {
                    (*clone.m_buffer).tot_len = (*original).len;
                    (*clone.m_buffer).len = (*original).len;
                    ptr::copy_nonoverlapping(
                        (*original).reserve_start(),
                        (*clone.get_raw()).reserve_start(),
                        original_data_size + (original_reserved_size as usize),
                    );
...
impl PacketBufferHandle {
    ...
    pub fn new(a_available_size: u32, a_reserved_size: u16) -> Option<Self> {
        let sum_of_sizes: u64 =
            a_available_size as u64 + a_reserved_size as u64 + PacketBuffer::KSTRUCTURESIZE as u64;
    ...
    pub fn retain(&mut self) -> Option<Self> {
        ...
        return Some(PacketBufferHandle {
...
mod test {
    ...
    mod new {
        ...
        fn new_packet_buffer_with_oversize_alloc_max_available_space() {
            ...
            let buffer = PacketBufferHandle::new(
                PacketBuffer::KMAX_ALLOC_SIZE / 2 + extra,
                (PacketBuffer::KMAX_ALLOC_SIZE / 2) as u16,
            );
            assert_eq!(buffer.is_none(), true);
        }
        ...
        fn push_one_and_clone() {
            ...
            unsafe {
                ...
                for i in 0..data2.len() {
                    assert_eq!(
                        ptr::read(next_buffer_ptr.add(i)),
                        ptr::read(next_clone_ptr.add(i))
                    );
                }
            }
        }
    ...
    mod advance_and_consume {
        ...
        fn ensure_reserved_size_too_big() {
            ...
            unsafe {
                assert_eq!(
                    false,
                    (*pb).ensure_reserved_size(PacketBuffer::KMAX_SIZE_WITHOUT_RESERVE as u16)
                );
            }
        }
