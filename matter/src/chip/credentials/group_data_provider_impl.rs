...
use crate::{
    chip::{
        ...
        credentials::group_data_provider::{
            GroupDataProvider, GroupListener, GroupInfo, GroupKey, KeySet, GroupEndpoint, GroupSession, key_set,
            KIDENTITY_PROTECTION_KEY_SET_ID,
        },
        crypto::{
...
pub mod key_map_data {
    ...
    pub type PersistentKeyMapData<S> = PersistentData<KeyMapData, K_PERSISTENT_BUFFER_MAX, S>;
...
pub mod key_set_data {
    ...
    */
    ...
    pub type PersistentKeySetData<S> = PersistentData<KeySetData, K_PERSISTENT_BUFFER_MAX, S>;
...
pub mod iter_impl {
    ...
    pub struct GroupKeyIteratorImpl<Provider: GroupDataProviderIterImpl>
    {
        m_provider: Option<NonNull<Provider>>,
        m_fabric: FabricIndex,
        m_next_id: u16,
    ...
    impl<Provider: GroupDataProviderIterImpl> GroupKeyIteratorImpl<Provider> {
        pub const fn new() -> Self {
            Self {
                m_provider: None,
                m_fabric: KUNDEFINED_FABRIC_INDEX,
        ...
        pub fn new_with(provider: Option<NonNull<Provider>>, fabric_index: FabricIndex) -> Option<Self> {
            ...
            let mut fabric: FabricData = fabric_data::new_with(fabric_index);
            if FabricData::load_from(&mut fabric, storage_ptr).is_ok() {
                s.m_next_id = fabric.first_map;
                s.m_total = usize::from(fabric.map_count);
                s.m_count = 0;
            }
    ...
    impl<Provider: GroupDataProviderIterImpl> Iterator for GroupKeyIteratorImpl<Provider> {
        type Item = GroupKey;
        ...
        fn next(&mut self) -> Option<Self::Item> {
            ...
            verify_or_return_error!(self.m_count < self.m_total, None);
            let mut map = key_map_data::new_with(self.m_fabric, self.m_next_id, KUNDEFINED_GROUP_ID, 0);
            KeyMapData::load_from(&mut map, storage_ptr).ok()?;

            self.m_count += 1;
            self.m_next_id = map.linked_data.next;

            Some(map.group_key.clone())
        }
    }

    impl<Provider: GroupDataProviderIterImpl> Release for GroupKeyIteratorImpl<Provider> {
        fn release(&mut self) {
            if let Some(ptr) = self.m_provider {
                unsafe {
                    ptr.as_ref().release_iter_group_key();
                }
            }
        }
    }

    impl<Provider: GroupDataProviderIterImpl> Drop for  GroupKeyIteratorImpl<Provider> {
        fn drop(&mut self) {
            self.release()
        }
    }
    ...
    impl<Provider: GroupDataProviderIterImpl + UpdateSessionKeystore> SymmetricKeyContext for GroupKeyContext<Provider>
    {
        ...
        fn release(&mut self) {
            ...
            if let Some(ptr) = self.m_provider {
                unsafe {
                    ptr.as_ref().release_key_context();
                }
            }
        }
    }

    impl<Provider: GroupDataProviderIterImpl + UpdateSessionKeystore> Drop for  GroupKeyContext<Provider> {
        fn drop(&mut self) {
            self.release()
        }
    }

    pub struct KeySetIteratorImpl<Provider: GroupDataProviderIterImpl>
    {
        m_provider: Option<NonNull<Provider>>,
        m_fabric: FabricIndex,
        m_next_id: u16,
    ...
    impl<Provider: GroupDataProviderIterImpl> KeySetIteratorImpl<Provider> {
        pub const fn new() -> Self {
            Self {
                m_provider: None,
                m_fabric: KUNDEFINED_FABRIC_INDEX,
    ...
    pub struct GroupSessionIteratorImpl<Provider: GroupDataProviderIterImpl + UpdateSessionKeystore>
    {
        m_provider: Option<NonNull<Provider>>,
        m_session_id: u16,
    ...
    impl<Provider: GroupDataProviderIterImpl + UpdateSessionKeystore> GroupSessionIteratorImpl<Provider>
    {
        pub const fn new() -> Self {
            Self {
                m_provider: None,
    ...
    impl<Provider: GroupDataProviderIterImpl + UpdateSessionKeystore> Iterator for GroupSessionIteratorImpl<Provider>
    {
        ...
        fn next(&mut self) -> Option<Self::Item> {
            ...
            while self.m_fabric_count < self.m_fabric_total {
                let mut fabric: FabricData = fabric_data::new_with(self.m_fabric);
                FabricData::load_from(&mut fabric, storage_ptr).ok()?;

                if self.m_map_count >= fabric.map_count {
                    // No more keyset/group mappings on the current fabric, try next fabric
                    self.m_fabric = fabric.next;
                    self.m_fabric_count += 1;
                    self.m_first_map = true;
                    self.m_map_count = 0;
                    continue;
                }

                if self.m_first_map {
                    self.m_mapping = fabric.first_map;
                    self.m_first_map = false;
                }

                let mut mapping: KeyMapData = key_map_data::new_with(self.m_fabric, self.m_mapping, KUNDEFINED_GROUP_ID, 0);
                KeyMapData::load_from(&mut mapping, storage_ptr).ok()?;

                // Group found, get the keyset
                let mut keyset: KeySetData = key_set_data::new();
                unsafe {
                    verify_or_return_error!(key_set_data::find(&mut keyset, 
                           NonNull::new_unchecked(storage_ptr), &fabric, mapping.group_key.keyset_id.into()), None);
                }

                if self.m_key_index >= keyset.keys_count.into() {
                    // No more keys in current keyset, try next
                    self.m_mapping = mapping.linked_data.next;
                    self.m_map_count += 1;
                    self.m_key_index = 0;
                    continue;
                }

                let creds = &keyset.operational_keys[usize::from(self.m_key_index)];
                self.m_key_index += 1;

                if creds.m_hash == self.m_session_id {
                    self.m_group_key_context.initialize(&creds.m_encryption_key, self.m_session_id, &creds.m_privacy_key);
                    let mut output = Self::Item::new();
                    output.fabric_index = fabric.fabric_index;
                    output.group_id = mapping.group_key.group_id;
                    output.security_policy = keyset.policy;
                    output.key_context = Some(NonNull::from_ref(&self.m_group_key_context));
    ...
    impl<Provider: GroupDataProviderIterImpl + UpdateSessionKeystore> Release for GroupSessionIteratorImpl<Provider> {
        fn release(&mut self) {
            if let Some(ptr) = self.m_provider {
                unsafe {
                    ptr.as_ref().release_iter_group_session();
                }
            }
        }
    }

    impl<Provider: GroupDataProviderIterImpl + UpdateSessionKeystore> Drop for  GroupSessionIteratorImpl<Provider> {
        fn drop(&mut self) {
            self.release()
        }
    }
...
type GroupInfoIterator<PSD, SKS> = iter_impl::GroupInfoIteratorImpl<GroupDataProviderImpl<PSD, SKS>>;

type GroupKeyIterator<PSD, SKS> = iter_impl::GroupKeyIteratorImpl<GroupDataProviderImpl<PSD, SKS>>;

type EndpointIterator<PSD, SKS> = iter_impl::EndpointIteratorImpl<GroupDataProviderImpl<PSD, SKS>>;

type KeySetIterator<PSD, SKS> = iter_impl::KeySetIteratorImpl<GroupDataProviderImpl<PSD, SKS>>;

type KeyContext<PSD, SKS> = iter_impl::GroupKeyContext<GroupDataProviderImpl<PSD, SKS>>;

type GroupSessionIterator<PSD, SKS> = iter_impl::GroupSessionIteratorImpl<GroupDataProviderImpl<PSD, SKS>>;
...
pub struct GroupDataProviderImpl<PSD, SKS>
where
...
impl<PSD, SKS> Storage for GroupDataProviderImpl<PSD, SKS>
where
    PSD: PersistentStorageDelegate,
    SKS: SessionKeystore,
{
...
impl<PSD, SKS> iter_impl::GroupDataProviderIterImpl for GroupDataProviderImpl<PSD, SKS>
where
    PSD: PersistentStorageDelegate,
    SKS: SessionKeystore,
{ }

impl<PSD, SKS> GroupDataProviderImpl<PSD, SKS>
where
    PSD: PersistentStorageDelegate,
    SKS: SessionKeystore,
{
    pub const fn new() -> Self {
        Self {
            ...
            m_group_key_context_pool: Cell::new(0),
    ...
    pub fn remove_endpoints(&mut self, fabric_index: FabricIndex, group_id: GroupId) -> ChipErrorResult {
        unsafe {
            ...
            for _endpoint_index in 0..group.endpoint_count {
    ...
    fn group_added(&mut self, fabric_index: FabricIndex, new_group: &GroupInfo) {
        unsafe {
            if let Some(listener_ptr) = self.m_listener &&
                let Some(listener_ref) = listener_ptr.as_mut()
            {
                    listener_ref.on_group_added(fabric_index, new_group);
            }
        }
    }
    ...
    fn group_removed(&mut self, fabric_index: FabricIndex, old_group: &GroupInfo) {
        unsafe {
            if let Some(listener_ptr) = self.m_listener &&
                let Some(listener_ref) = listener_ptr.as_mut()
            {
                    listener_ref.on_group_removed(fabric_index, old_group);
            }
        }
    }

    fn remove_endpoint_at(&mut self, fabric_index: FabricIndex, group_id: GroupId, endpoint_id: EndpointId) -> ChipErrorResult {
        let storage_ptr = 
        ...
        return self.remove_group_info_at(fabric_index, group.index.into());
    }
}

impl<PSD, SKS> GroupDataProvider for GroupDataProviderImpl<PSD, SKS>
where
    PSD: PersistentStorageDelegate,
    SKS: SessionKeystore,
{
    type GroupInfoIterator = GroupInfoIterator<PSD, SKS>;
    type GroupKeyIterator = GroupKeyIterator<PSD, SKS>;
    type EndpointIterator = EndpointIterator<PSD, SKS>;
    type KeySetIterator = KeySetIterator<PSD, SKS>;
    type GroupSessionIterator = GroupSessionIterator<PSD, SKS>;
    type KeyContext = KeyContext<PSD, SKS>;
    ...
    fn finish(&mut self) {
        ...
        self.m_group_key_context_pool.set(0);
    }
    ...
    fn set_group_info_at(&mut self, fabric_index: FabricIndex, index: usize, info: &GroupInfo) -> ChipErrorResult {
        unsafe {
            ...
            // Update fabric
            fabric_data::save(&mut fabric, self.m_storage.clone().unwrap())?;
    ...
    fn remove_endpoint(&mut self, fabric_index: FabricIndex, group_id: Option<GroupId>, endpoint_id: EndpointId) -> ChipErrorResult {
        if let Some(gid) = group_id {
            return self.remove_endpoint_at(fabric_index, gid, endpoint_id);
        }
        ...
        for _group_index in 0..fabric.group_count {
            if GroupData::load_from(&mut group, storage_ptr).is_err() {
                break;
            }
            if endpoint_data::find(&mut endpoint, self.m_storage.clone().unwrap(), &fabric, &group, endpoint_id) {
                self.remove_endpoint_at(fabric_index, group.group_info.group_id, endpoint_id)?;
            }
    ...
    // Group-Key map
    //
    fn set_group_key_at(&mut self, fabric_index: FabricIndex, index: usize, in_map: &GroupKey) -> ChipErrorResult {
        let storage_ptr = 
            self.m_storage.as_ref().ok_or(chip_error_internal!())?.as_ptr();

        let mut fabric: FabricData = fabric_data::new_with(fabric_index);
        let mut map: KeyMapData = key_map_data::new_with_fabric(fabric_index);
        ...
        let found = key_map_data::find_by_key(&mut map, self.m_storage.clone().unwrap(), &fabric, &in_map);

        verify_or_return_error!(!found || (usize::from(map.linked_data.index) == index), Err(chip_error_duplicate_key_id!()));

        let found = key_map_data::get(&mut map, self.m_storage.clone().unwrap(), &fabric, index);
        map.group_key = in_map.clone();

        if found {
            // Update existing map
            return KeyMapData::save_to(&mut map, storage_ptr);
        }

        // Insert last
        verify_or_return_error!(usize::from(fabric.map_count) == index, Err(chip_error_invalid_argument!()));
        verify_or_return_error!(fabric.map_count < self.m_max_groups_per_fabric, Err(chip_error_invalid_list_length!()));

        map.linked_data.next = 0;
        KeyMapData::save_to(&mut map, storage_ptr)?;

        if map.linked_data.first {
            // First map, update fabric
            fabric.first_map = map.linked_data.id;
        } else {
            // Last map, update previous
            let mut prev: KeyMapData = key_map_data::new_with(fabric_index, map.linked_data.prev, KUNDEFINED_GROUP_ID, 0);
            KeyMapData::load_from(&mut prev, storage_ptr)?;
            prev.linked_data.next = map.linked_data.id;
            KeyMapData::save_to(&mut prev, storage_ptr)?;
        }
    ...
    fn get_group_key_at(&self, fabric_index: FabricIndex, index: usize) -> Result<GroupKey, ChipError> {
        ...
        FabricData::load_from(&mut fabric, storage_ptr)?;
        verify_or_return_error!(key_map_data::get(&mut map, self.m_storage.clone().unwrap(), &fabric, index), Err(chip_error_not_found!()));

        // Target map found
        Ok(map.group_key.clone())
    }
    ...
    fn remove_group_key_at(&mut self, fabric_index: FabricIndex, index: usize) -> ChipErrorResult {
        ...
        if map.linked_data.first {
            fabric.first_map = map.linked_data.next;
        } else {
            let mut prev: KeyMapData = key_map_data::new_with(fabric_index, map.linked_data.prev, KUNDEFINED_GROUP_ID, 0);
            KeyMapData::load_from(&mut prev, storage_ptr)?;
            prev.linked_data.next = map.linked_data.next;
            KeyMapData::save_to(&mut prev, storage_ptr)?;
        }
    ...
    fn remove_group_keys(&mut self, fabric_index: FabricIndex) -> ChipErrorResult {
        ...
        let mut map: KeyMapData = key_map_data::new();
        for _count in 0..fabric.map_count {
            if KeyMapData::load_from(&mut map, storage_ptr).is_err() {
                break;
            }
    ...
    fn iter_group_keys(&self, fabric_index: FabricIndex) -> Option<Self::GroupKeyIterator> {
        verify_or_return_error!(self.is_initialized(), None);
        verify_or_return_error!(self.m_group_key_iterators.get() < iter_impl::GROUP_KEY_MAX, None);

        let c = self.m_group_key_iterators.get();
        self.m_group_key_iterators.set(c + 1);

        Some(GroupKeyIterator::new_with(Some(NonNull::from_ref(self)), fabric_index)?)
    }
    ...
    // Key Sets
    //
    fn set_key_set(&mut self, fabric_index: FabricIndex, compressed_fabric_id: &[u8], in_keyset: &KeySet) -> ChipErrorResult {
        let storage_ptr = 
            self.m_storage.as_ref().ok_or(chip_error_internal!())?.as_ptr();
        ...
        let found = key_set_data::find(&mut keyset, self.m_storage.clone().unwrap(), &fabric, in_keyset.keyset_id.into());

        keyset.keyset_id = in_keyset.keyset_id;
        keyset.policy = in_keyset.policy;
        keyset.keys_count = in_keyset.num_keys_used;
        keyset.operational_keys = [const { GroupOperationalCredentials::new() }; key_set::KEPOCH_KEYS_MAX];
        keyset.operational_keys[0].m_start_time = in_keyset.epoch_keys[0].start_time;
        keyset.operational_keys[1].m_start_time = in_keyset.epoch_keys[1].start_time;
        keyset.operational_keys[2].m_start_time = in_keyset.epoch_keys[2].start_time;

        // Store the operational keys and hash instead of the epoch keys
        for i in 0..in_keyset.num_keys_used as usize{
    ...
    fn get_ipk_key_set(&self, fabric_index: FabricIndex) -> Result<KeySet, ChipError> {
        ...
        let mut _mapping: KeyMapData = key_map_data::new_with(fabric.fabric_index, fabric.first_map, KUNDEFINED_GROUP_ID, 0);

        // Fabric found, get the keyset
        let mut keyset: KeySetData = key_set_data::new();
        verify_or_return_error!(key_set_data::find(&mut keyset, self.m_storage.clone().unwrap(), 
                &fabric, KIDENTITY_PROTECTION_KEY_SET_ID.into()), Err(chip_error_not_found!()));

        // If the keyset ID doesn't match, we have a ... problem.
        verify_or_return_error!(keyset.keyset_id == KIDENTITY_PROTECTION_KEY_SET_ID, Err(chip_error_internal!()));

        let mut out_keyset: KeySet = KeySet::new();
        out_keyset.keyset_id = keyset.keyset_id;
        out_keyset.num_keys_used = keyset.keys_count;
        out_keyset.policy = keyset.policy;

        for key_idx in 0..key_set::KEPOCH_KEYS_MAX {
            out_keyset.epoch_keys[key_idx].key.fill(0);
            if key_idx < keyset.keys_count.into() {
                out_keyset.epoch_keys[key_idx].start_time = keyset.operational_keys[key_idx].m_start_time;
                out_keyset.epoch_keys[key_idx].key.copy_from_slice(&keyset.operational_keys[key_idx].m_encryption_key[..]);
            }
        }
    ...
    fn remove_fabric(&mut self, fabric_index: FabricIndex) -> ChipErrorResult {
        ...
        for _keyset_count in 0..fabric.keyset_count {
    ...
    fn iter_group_session(&self, session_id: u16) -> Option<Self::GroupSessionIterator> {
        verify_or_return_error!(self.is_initialized(), None);
        verify_or_return_error!(self.m_group_session_iterators.get() < iter_impl::GROUP_SESSION_MAX, None);

        let c = self.m_group_session_iterators.get();
        self.m_group_session_iterators.set(c + 1);

        Some(GroupSessionIterator::new_with(Some(NonNull::from_ref(self)), session_id)?)
    }
    ...
    fn get_key_context(&mut self, fabric_index: FabricIndex, group_id: GroupId) -> Result<Self::KeyContext, ChipError> {
        ...
        verify_or_return_error!(self.m_group_key_context_pool.get() < iter_impl::GROUP_KEY_CONTEXT_MAX, Err(chip_error_invalid_list_length!()));

        let mut fabric: FabricData = fabric_data::new_with(fabric_index);
        FabricData::load_from(&mut fabric, storage_ptr)?;

        let mut mapping = key_map_data::new_with(fabric.fabric_index, fabric.first_map, KUNDEFINED_GROUP_ID, 0);

        // Look for the target group in the fabric's keyset-group pairs
        for _i in 0..fabric.map_count {
            KeyMapData::load_from(&mut mapping, storage_ptr)?;
            // GroupKeySetID of 0 is reserved for the Identity Protection Key (IPK),
            // it cannot be used for operational group communication.

            if mapping.group_key.keyset_id > 0 && mapping.group_key.group_id == group_id {
                // Group found, get the keyset
                let mut keyset: KeySetData = key_set_data::new();
                verify_or_return_error!(key_set_data::find(&mut keyset, self.m_storage.clone().unwrap(), 
                        &fabric, mapping.group_key.keyset_id.into()), Err(chip_error_not_found!()));
                if let Some(creds) = keyset.get_current_group_credentials() {
                    let c = self.m_group_key_context_pool.get();
                    self.m_group_key_context_pool.set(c + 1);

                    return Ok(Self::KeyContext::new_with(
                            Some(NonNull::from_ref(self)), &creds.m_encryption_key, creds.m_hash, &creds.m_privacy_key));
                }
            }

            mapping.linked_data.id = mapping.linked_data.next;
        }
...
mod tests {
    ...
    #[test]
    fn remove_endpoint_successfully() {
        let pa = TestPersistentStorage::default();
        let ks = RawKeySessionKeystore::new();
        let mut l = TestGroupListener::new();
        let mut p = <TestGroupDataProvider as GroupDataProvider>::new();
        ...
        assert!(p.remove_endpoint_at(fabric_index, group_id, endpoint_id).is_ok());
    }

    #[test]
    fn remove_endpoint_no_endpoint() {
        let pa = TestPersistentStorage::default();
        let ks = RawKeySessionKeystore::new();
        let mut p = <TestGroupDataProvider as GroupDataProvider>::new();
        let fabric_index: FabricIndex = 1;
    ...
    #[test]
    fn remove_two_endpoints_successfully() {
        let pa = TestPersistentStorage::default();
        let ks = RawKeySessionKeystore::new();
        let mut l = TestGroupListener::new();
        let mut p = <TestGroupDataProvider as GroupDataProvider>::new();
        ...
        assert!(p.remove_endpoint_at(fabric_index, group_id, endpoint_id + 1).is_ok());
        assert!(p.remove_endpoint_at(fabric_index, group_id, endpoint_id).is_ok());
    }

    #[test]
    fn remove_middle_endpoint_successfully() {
        let pa = TestPersistentStorage::default();
        let ks = RawKeySessionKeystore::new();
        let mut l = TestGroupListener::new();
        let mut p = <TestGroupDataProvider as GroupDataProvider>::new();
        ...
        assert!(p.add_endpoint(fabric_index, group_id, endpoint_id).is_ok());
        assert!(p.add_endpoint(fabric_index, group_id, endpoint_id + 1).is_ok());
        assert!(p.add_endpoint(fabric_index, group_id, endpoint_id + 2).is_ok());
        assert!(l.last_add.is_some_and(|(f, g)| f == fabric_index && g.group_id == group_id));

        assert!(p.remove_endpoint_at(fabric_index, group_id, endpoint_id + 1).is_ok());
        assert!(p.has_endpoint(fabric_index, group_id, endpoint_id));
        assert!(p.has_endpoint(fabric_index, group_id, endpoint_id + 2));
    }
    ...
    fn set_group_key_differnt_key_to_same_index() {
        ...
        assert!(p.set_group_key_at(fabric_index, 0, &group_key).is_ok());
        assert!(p.set_group_key_at(fabric_index, 0, &group_key_2).is_ok());
    }
    ...
    fn set_group_key_at_set_2_keys() {
        ...
        assert!(p.set_group_key_at(fabric_index, 0, &group_key).is_ok());
        assert!(p.set_group_key_at(fabric_index, 1, &group_key_2).is_ok());
    }
    ...
    fn get_group_key_at_index_outof_boundary() {
        ...
        assert!(p.set_group_key_at(fabric_index, 0, &group_key).is_ok());
        assert!(!p.get_group_key_at(fabric_index, 1).is_ok());
    }
    ...
    fn remove_group_key_at_successsfully() {
        ...
        assert!(p.set_group_key_at(fabric_index, 0, &group_key).is_ok());
        assert!(p.remove_group_key_at(fabric_index, 0).is_ok());
    }
    ...
    fn remove_group_key_at_no_key() {
        ...
        assert!(p.set_group_key_at(fabric_index, 0, &group_key).is_ok());
        assert!(!p.remove_group_key_at(fabric_index, 1).is_ok());
    }
    ...
    fn remove_group_keys_successfully() {
        ...
        assert!(p.set_group_key_at(fabric_index, 0, &group_key).is_ok());
        assert!(p.set_group_key_at(fabric_index, 1, &group_key_2).is_ok());
        assert!(p.remove_group_keys(fabric_index).is_ok());
    }
    ...
    fn iter_group_key() {
        ...
        assert!(p.set_group_key_at(fabric_index, 1, &group_key_2).is_ok());
        {
            let key = p.iter_group_keys(fabric_index);
            assert_eq!(1, p.m_group_key_iterators.get());
            assert!(key.is_some());
            let mut key = key.unwrap();
            assert!(Some(group_key.clone()) == key.next());
            assert!(Some(group_key_2.clone()) == key.next());
    ...
    fn get_group_key_context_too_many() {
        ...
        let k = p.get_key_context(fabric_index, group_id);
        let k_2 = p.get_key_context(fabric_index, group_id);
        assert!(k.is_ok());
        assert!(k_2.is_ok());
        assert!(!p.get_key_context(fabric_index, group_id).is_ok());
    }
    ...
    fn get_group_key_context_incorrect_group_id() {
        ...
        assert!(!p.get_key_context(fabric_index, group_id + 1).is_ok());
    }
    ...
    fn get_ipk_key_successfully() {
        ...
        assert!(p.get_ipk_key_set(fabric_index).is_ok_and(|k| k.keyset_id == KIDENTITY_PROTECTION_KEY_SET_ID && k.num_keys_used == 2));
    }
    ...
    fn get_ipk_key_no_key() {
        ...
        // create keyset
        let keyset = KeySet::new_with(keyset_id + 1, SecurityPolicy::KcacheAndSync, 2);
        let compressed_fabric_id = u16::to_be_bytes(1u16);
        assert!(p.set_key_set(fabric_index, &compressed_fabric_id[..], &keyset).is_ok());
    ...
    fn iter_group_session_successfully() {
        ...
        // get session id
        {
            ...
            assert!(key.is_ok());
            let mut key = key.unwrap();
            session_id = key.get_key_hash();
        }

        {
            let s = p.iter_group_session(session_id);
            assert!(s.is_some());
            let mut s = s.unwrap();
            unsafe {
