    ...
    DelegateType: RawTransportDelegate,
{
    ...
    fn init(&mut self, params: Self::InitParamType) -> ChipError {
        ...
        unsafe {
            ...
            let err = (*self.m_test_end_point).bind(
                params.get_address_type(),
                &IPAddress::ANY.clone(),
                params.get_listen_port(),
            );
            ...
            let err = (*self.m_test_end_point).listen(
                ...
                Some(|ep, buffer, pkt_info| {
                    ...
                    let peer_address = PeerAddress::udp_addr_port_interface(
                        pkt_info.src_address.clone(),
                        pkt_info.src_port,
                        pkt_info.interface.unwrap_or(InterfaceId::default()),
                    );
            ...
            chip_log_detail!(
                Inet,
                "Test::Inet bound to port={}",
                (*self.m_test_end_point).get_bound_port()
            );
        }
    ...
    DelegateType: RawTransportDelegate,
{
    ...
    fn send_message(
        ...
        verify_or_return_error!(
            self.m_test_end_point.is_null() == false,
            chip_error_incorrect_state!()
        );
    ...
    fn handle_message_received(
        &mut self,
        peer_address: PeerAddress,
        buffer: PacketBufferHandle,
        ctxt: *const MessageTransportContext,
    ) {
        unsafe { (*self.m_delegate).handle_message_received(peer_address, buffer, ctxt) }
    }
}
...
mod test {
    ...
    mod test_listen_parameter {
        ...
        fn new_with_end_point_manager() {
            ...
            assert_eq!(
                p.get_end_point_manager(),
                ptr::addr_of_mut!(END_POINT_MANAGER)
            );
        }
    ...
    mod test_transport_init {
        ...
        fn init() {
            ...
            unsafe {
                assert_eq!(
                    the_test.init(TEST_PARAMS.assume_init_mut().clone()),
                    chip_no_error!()
                );
            }
        ...
        fn init_fail_at_listen() {
            ...
            // fail at checked = 1
            let _ =
                get_manager().fail_at_fault(InetFaultInjectionID::KFaultListen as Identifier, 0, 1);
            unsafe {
                assert_eq!(
                    the_test.init(TEST_PARAMS.assume_init_mut().clone()),
                    chip_error_incorrect_state!()
    ...
    mod test_transport_send {
        ...
        */
        ...
        const EXPECTED_SEND_ADDR: IPAddress = IPAddress { addr: (1, 2, 3, 4) };
        ...
        fn on_send_checked(info: IPPacketInfo, mut buffer: PacketBufferHandle) -> ChipError {
            ...
            if let Some(msg) = buffer.pop_head() {
                ...
                unsafe {
                    ...
                    for i in 0..4 {
                        assert_eq!(
                            ptr::read(buffer_ptr.add(i)),
                            ptr::read(expected_data_ptr.add(i))
                        );
                    }
                }
            } else {
    ...
    mod test_transport_receive {
        ...
        fn receive_successfully() {
            ...
            unsafe {
                ...
                let msg =
                    PacketBufferHandle::new_with_data(&EXPECTED_SEND_MSG[0..4], 0, 0).unwrap();
