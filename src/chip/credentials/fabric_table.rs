...
pub mod fabric_info {
    ...
    */
    ...
    pub(super) struct InitParams {
        ...
        pub m_operation_key: Option<NonNull<P256Keypair>>,
    }
    ...
    pub struct FabricInfo {
        ...
        m_operation_key: Option<Keypair>,
    }
    ...
    */
    ...
    impl FabricInfo {
        ...
        pub(super) fn set_operational_keypair(
            &mut self,
            keypair: NonNull<P256Keypair>,
        ) -> ChipErrorResult {
            let mut serialized = P256SerializedKeypair::default();

            unsafe {
                keypair.as_ref().serialize(&mut serialized)?;
            }
        ...
        pub(super) fn sign_with_op_keypair(
            ...
            match self.m_operation_key.as_ref() {
                ...
                Some(Keypair::External(keypair)) => {
                    unsafe {
                        keypair.as_ref().ecdsa_sign_msg(message, out_signature)
                    }
                }
                None => {
                    Err(chip_error_key_not_found!())
...
mod fabric_table {
    ...
    use crate::chip_error_fabric_exists;
    use crate::chip_ok;
    use crate::chip_sdk_error;
    ...
    pub trait Delegate<'a, PSD, OK, OCS>
    where
        ...
        OCS: credentials::OperationalCertificateStore,
    {
        ...
        fn next(&self) -> Option<*mut (dyn Delegate<'a, PSD, OK, OCS> + 'a)>;
        fn remove_next(&mut self);
        fn set_next(&mut self, next: Option<*mut (dyn Delegate<'a, PSD, OK, OCS> + 'a)>);
    }
    ...
    pub struct FabricTable<'d, PSD, OK, OCS>
    where
        PSD: PersistentStorageDelegate,
        OK: crypto::OperationalKeystore,
        OCS: credentials::OperationalCertificateStore,
    {
        ...
        m_delegate_list_root: Option<*mut (dyn Delegate<'d, PSD, OK, OCS> + 'd)>,
    ...
    */
    ...
    impl<PSD, OK, OCS> FabricTable<'_, PSD, OK, OCS>
    where
        PSD: PersistentStorageDelegate,
        OK: crypto::OperationalKeystore,
        OCS: credentials::OperationalCertificateStore,
    {
        ...
        pub fn init(&mut self, init_params: &InitParams<PSD, OK, OCS>) -> ChipErrorResult {
            ...
            let _ = self.m_last_known_good_time.init(self.m_storage).inspect_err(|e| chip_log_progress!(FabricProvisioning, "Cannot init last known good time {}", e));
            ...
            match self.get_commit_marker() {
                Ok(commit_marker) => {
                    ...
                    let _ = self.delete(commit_marker.fabric_index).inspect_err(|e| chip_log_error!(FabricProvisioning, "Cannot delete fabric {:#x}: {}",
                            commit_marker.fabric_index, e));
                }
                Err(e) => {
    ...
    impl<'d, PSD, OK, OCS> FabricTable<'d, PSD, OK, OCS>
    where
        PSD: PersistentStorageDelegate,
        OK: crypto::OperationalKeystore,
        OCS: credentials::OperationalCertificateStore,
    {
        pub fn delete(&mut self, fabric_index: FabricIndex) -> ChipErrorResult {
            ...
            if fabric_is_initialized {
                ...
                // index.
                let _ = self.store_fabric_index_info().inspect_err(|e| chip_log_progress!(FabricProvisioning, "Cannot store fabric index info {}", e));
        ...
        pub fn delete_all_fabric(&mut self) {
            ...
            for (index, deleted) in to_be_deleted.into_iter().enumerate() {
                if deleted {
                    let _ = self.delete(index as u8).inspect_err(|e| chip_log_error!(FabricProvisioning, "Cannot delete fabric at {}: {}", index, e));
                }
            }
        }
        ...
        pub fn clear_commit_marker(&mut self) {
            ...
            unsafe {
                let _ = self.m_storage.as_mut().unwrap().sync_delete_key_value(
                    DefaultStorageKeyAllocator::fabric_table_commit_marker_key().key_name_str(),
                ).inspect_err(|e| chip_log_error!(FabricProvisioning, "Cannot clear commit marker {}", e));
            }
        }
        ...
        pub fn add_fabric_delegate(
            &mut self,
            delegate: Option<*mut (dyn Delegate<'d, PSD, OK, OCS> + 'd)>,
        ) -> ChipErrorResult {
        ...
        pub fn remove_fabric_delegate(
            &mut self,
            mut delegate: Option<*mut (dyn Delegate<'d, PSD, OK, OCS> + 'd)>,
        ) {
            if delegate.is_none() {
                return;
            }
        ...
        pub fn fetch_icac_cert(
            ...
            unsafe {
                match self.m_op_cert_store.as_ref().unwrap().get_certificate(
                    ...
                    Ok(size) => {
                        out_cert.set_length(size).map_err(|_| chip_error_buffer_too_small!())?;
                        return chip_ok!();
                    }
                    Err(e) => {
                        if e == chip_error_not_found!() {
                            ...
                            {
                                out_cert.set_length(0).map_err(|_| chip_error_buffer_too_small!())?;
                                return chip_ok!();
                            }
                        }
                        return Err(e);
        ...
        pub fn fetch_vid_verification_statement(
            ...
            unsafe {
                if let Some(store) = self.m_op_cert_store.as_ref() {
                    ...
                    out_vid_verification_statement.set_length(size).map_err(|_| chip_error_buffer_too_small!())?;
                    return chip_ok!();
                } else {
        ...
        pub fn fetch_vvsc(
            ...
            unsafe {
                if let Some(store) = self.m_op_cert_store.as_ref() {
                    ...
                    out_vvsc.set_length(size).map_err(|_| chip_error_buffer_too_small!())?;
                    return chip_ok!();
                } else {
                    return Err(chip_error_incorrect_state!());
                }
        ...
        pub fn add_new_pending_fabric_with_provided_op_key(
            ...
            icac: &[u8],
            vendor_id: u16,
            existeding_op_key: Option<NonNull<P256Keypair>>,
        ...
        pub fn commit_pending_fabric_data(&mut self) -> ChipErrorResult {
...

            {
                ...
                sticky_error = sticky_error.and(self.store_fabric_metadata(live_fabric_entry).inspect_err(|e| {
                ...
                unsafe {
                    if let Some(key_store) = self.m_operational_keystore.as_mut() {
                        if key_store.has_op_keypair_for_fabric(fabric_index_begin_committed) && key_store.has_pending_op_keypair() {
                            sticky_error = sticky_error.and(key_store.commit_op_keypair_for_fabric(fabric_index_begin_committed).inspect_err(|e| {
                    ...
                    if let Some(op_store) = self.m_op_cert_store.as_mut() {
                        sticky_error = sticky_error.and(op_store.commit_certs_for_fabric(fabric_index_begin_committed).inspect_err(|e| {
                ...
                let _ = self.m_last_known_good_time.commit_last_known_good_chip_epoch_time().inspect_err(|e| {
                ...
                if self.m_state_flag.intersects(StateFlags::KisAddPending) {
                    ...
                    sticky_error = sticky_error.and(self.store_fabric_index_info().inspect_err(|e| {
            ...
            if sticky_error.is_err() {
                ...
                let _ = self.delete(fabric_index_begin_committed).inspect_err(|e|
                    chip_log_error!(FabricProvisioning, "Cannot delete fabric {:#x}: {}", fabric_index_begin_committed, e));

                self.revert_pending_fabric_data();
            } else {
                let _ = self.notify_fabric_commited(fabric_index_begin_committed);
            }
        ...
        pub fn revert_pending_fabric_data(&mut self) {
            ...
            let _ = self.m_last_known_good_time
        ...
        pub fn revert_pending_op_certs_except_root(&mut self) {
            ...
            if self.m_state_flag.contains(StateFlags::KisAddPending) {
                let _ = self.delete(self.m_fabric_index_with_pending_state).inspect_err(|e| 
                    chip_log_error!(FabricProvisioning, "Cannot delete fabric {:#x}: {}", self.m_fabric_index_with_pending_state, e));
            }
        ...
        fn add_or_update_inner(
            ...
            fabric_index: FabricIndex,
            is_addition: bool,
            existing_op_key: Option<NonNull<P256Keypair>>,
            is_existing_op_key_externally_owned: bool,
            vendor_id: u16,
            advertise_identity: AdvertiseIdentity,
        ) -> ChipErrorResult {
            ...
            if let Some(existing_op_key_ref) = existing_op_key {
                ...
                unsafe{
                    verify_or_return_error!(existing_op_key_ref.as_ref().public_key().matches(&noc_public_key), Err(chip_error_invalid_public_key!()));
                }
            ...
            // Time to be reverted.
            let _ = self.m_last_known_good_time.update_pending_last_known_good_chip_epoch_time(not_before_collector.m_latest_not_before.get()).
                inspect_err(|e| {
        ...
        fn add_new_pending_fabric_common(
            ...
            verify_or_return_error!(!self.m_op_cert_store.is_null(), Err(chip_error_incorrect_state!()));
            chip_static_assert!(KMAX_VALID_FABRIC_INDEX <= u8::MAX, "Cannot create more fabrics than u8 MAX");
            ...
            if existeding_op_key.is_none() {
                ...
                unsafe {
                    if let Some(keystore) = self.m_operational_keystore.as_ref() {
                        verify_or_return_error!(keystore.has_op_keypair_for_fabric(fabric_index_to_use) ||
                            keystore.has_pending_op_keypair(), Err(chip_error_key_not_found!()));
                    } else {
            ...
            if !self.m_state_flag.intersects(StateFlags::KareCollidingFabricsIgnored) &&
                self.find_existing_fabric_by_noc_chaining(fabric_index_to_use, noc)?.is_some() 
            {
                return Err(chip_error_fabric_exists!());
            }

            unsafe {
                if let Some(op_store) = self.m_op_cert_store.as_mut() {
                    op_store.add_new_op_certs_for_fabric(fabric_index_to_use, noc, icac)?;
                } else {
                    // shoud not reach here since we check the null pointer
                    return Err(chip_error_incorrect_state!());
                }
            }
            ...
            self.add_or_update_inner(fabric_index_to_use, /* is_addition */ true, existeding_op_key, is_existing_op_key_externally_owned, vendor_id, advertise_identity)
                .inspect_err(|_| self.revert_pending_op_certs_except_root() )?;


            self.m_state_flag.insert(StateFlags::KisAddPending | StateFlags::KisPendingFabricDataPresent);

            // Notify that NOC was added (at least transiently)
            let _ = self.notify_fabric_updated(fabric_index_to_use);

            return Ok(fabric_index_to_use);
        }
        ...
        fn update_pending_fabric_common(
            ...
            // Notify that NOC was added (at least transiently)
            let _ = self.notify_fabric_updated(fabric_index);
    ...
    impl<'a, 'd, PSD, OK, OCS> IntoIterator for &'a FabricTable<'d, PSD, OK, OCS>
    where
        PSD: PersistentStorageDelegate,
        OK: crypto::OperationalKeystore,
        OCS: credentials::OperationalCertificateStore,
    ...
    impl<'a, 'd, PSD, OK, OCS> IntoIterator for &'a mut FabricTable<'d, PSD, OK, OCS>
    where
        PSD: PersistentStorageDelegate,
        OK: crypto::OperationalKeystore,
        OCS: credentials::OperationalCertificateStore,
    ...
    mod tests {
        ...
        #[derive(Default)]
        struct TestFabricTableDelegate<'a, PSD, OK, OCS>
        where
            PSD: PersistentStorageDelegate,
            OK: crypto::OperationalKeystore,
            OCS: credentials::OperationalCertificateStore,
        {
            ...
            pub next: Option<*mut (dyn Delegate<'a, PSD, OK, OCS> + 'a)>,
        }

        type NoMockFabricTableDelegate<'a> = TestFabricTableDelegate<'a, TestPersistentStorage, OK, OCS>;

        impl<'a, PSD, OK, OCS> Delegate<'a, PSD, OK, OCS> for TestFabricTableDelegate<'a, PSD, OK, OCS>
        where
            PSD: PersistentStorageDelegate,
            OK: crypto::OperationalKeystore,
            OCS: credentials::OperationalCertificateStore,
        ...
        fn set_up_stub_fabric(
            ...
            let info = get_stub_fabric_info_with_index(fabric_index);
            unsafe {
        ...
        */
        ...
        fn find_existing_fabric_by_noc_successfully() {
            ...
            // this is the fabric_id used in make_x509_cert_chain_2
            ...
            init_pas.m_root_publick_key = P256PublicKey::default_with_raw_value(&rcac_cert.m_public_key);
        ...
        #[test]
        fn add_new_pending_fabric_without_external_keypair_successfully() {
            let mut pa = TestPersistentStorage::default();
            let mut ks = OK::default();
            let mut pos = OCS::default();
            ...
            assert!(
                table.add_new_pending_fabric_common(noc_buf.const_bytes(), icac_buf.const_bytes(), 0x1u16, None, false, AdvertiseIdentity::Yes).inspect_err(|e| println!("err {}", e)).is_ok());
            assert_eq!(1, table.fabric_count());

        }

        #[test]
        fn add_new_pending_fabric_with_external_keypair_collided() {
            let (_, rcac_buf, _, _, icac_buf, _, _, noc_buf, noc_keypair) = make_x509_cert_chain_3_with_keypair();
        ...
        #[test]
        fn add_new_pending_fabric_with_external_keypair_mismatched_certs() {
            let (_, rcac_buf, _, _, _, _, _, _, _) = make_x509_cert_chain_3_with_keypair();
            let (_, _, _, _, icac_buf, _, _, noc_buf, noc_keypair) = make_x509_cert_chain_3_with_keypair();
        ...
        fn commit_pending_fabric_pending_fabric_not_inited() {
            ...
            table.m_state_flag.insert(StateFlags::KisUpdatePending | StateFlags::KisPendingFabricDataPresent);
            table.m_state_flag.insert(StateFlags::KisTrustedRootPending);
            assert!(!table.commit_pending_fabric_data().is_ok());
        }
        ...
        fn commit_pending_fabric_missing_keypair() {
            ...
            assert!(!table.commit_pending_fabric_data().is_ok());
            assert!(table.m_state_flag.is_empty());
        }
        ...
        #[test]
        fn commit_new_pending_fabric_without_external_keypair_successfully() {
            let mut pa = TestPersistentStorage::default();
            let mut ks = OK::default();
            let mut pos = OCS::default();
        ...
        #[test]
        fn commit_new_pending_fabric_without_external_keypair_not_able_commit_keypair() {
            let mut pa = TestPersistentStorage::default();
            let mut ks = OK::default();
            let mut pos = OCS::default();
            ...
            assert!(
                table.add_new_pending_fabric_common(noc_buf.const_bytes(), icac_buf.const_bytes(), 0x1u16, None, false, AdvertiseIdentity::Yes).inspect_err(|e| println!("err {}", e)).is_ok());
            assert_eq!(1, table.fabric_count());
            pa.add_posion_key(DefaultStorageKeyAllocator::fabric_op_key(fabric_index).key_name_str());
            assert!(!table.commit_pending_fabric_data().is_ok());
        }
