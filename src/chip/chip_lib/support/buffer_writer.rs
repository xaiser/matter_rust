    ...
    EndianPutter: EndianPut + Default,
{
    ...
    fn fit(&self) -> Result<usize, usize> {
        let actually_written = if self.m_buf.len() >= self.m_needed {
            self.m_needed
        } else {
            self.m_buf.len()
        };
    ...
    fn endian_sign_put(&mut self, x: i64, len: usize) -> &mut Self {
        ...
        if available > 0 {
            ...
            let _ =
                self.m_endian_putter
                    .endian_sign_put(x, copy_len, &mut self.m_buf[self.m_needed..]);
        }
        self.m_needed += len;
        self
    }
    ...
    fn endian_unsign_put(&mut self, x: u64, len: usize) -> &mut Self {
        ...
        if available > 0 {
            let copy_len: usize = min(available as usize, len);
            let _ = self.m_endian_putter.endian_unsign_put(
                x,
                copy_len,
                &mut self.m_buf[self.m_needed..],
            );
        }
        self.m_needed += len;
        self
    ...
    EndianPutter: EndianPut + Default,
{
    ...
    pub fn put_i16(&mut self, x: i16) -> &mut Self {
        ...
        if self.available() > 0 {
            let _ = self.m_endian_putter.endian_sign_put(
                x.into(),
                len,
                &mut self.m_buf[self.m_needed..],
            );
...
mod test {
    ...
    mod common_apis {
        ...
        fn init() {
            let mut buf: [u8; 1] = [0];
            let wb: little_endian::BufferWriter =
                little_endian::BufferWriter::default_with_buf(&mut buf[..]);
            assert_eq!(1, wb.size());
        }
        ...
        fn skip() {
            let mut buf: [u8; 1] = [0];
            let mut wb: little_endian::BufferWriter =
                little_endian::BufferWriter::default_with_buf(&mut buf[..]);
    ...
    mod big_endian_writer {
        ...
        fn put_u16() {
            let mut buf: [u8; 2] = [0; 2];
            let mut wb: big_endian::BufferWriter =
                big_endian::BufferWriter::default_with_buf(&mut buf[..]);
            let src: u16 = 0x1234;
            wb.put_u16(src);
