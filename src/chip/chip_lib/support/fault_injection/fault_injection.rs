...
impl Manager {
    ...
    pub fn init(
        &mut self,
        in_num_faults: usize,
        in_fault_array: &'static mut [Record],
        in_manager_name: &'static str,
        in_fault_names: &'static [&'static str],
    ) -> FaultInjectionResult {
        let mut err: FaultInjectionResult = Ok(());
        verify_or_return_value!(
            in_num_faults > 0
                && in_fault_array.is_empty() == false
                && in_manager_name.is_empty() == false
                && in_fault_names.is_empty() == false,
            err,
            err = Err(ErrorCode::KErrInvalid)
        );
    ...
    pub fn fail_randomly_at_fault(
        ...
        verify_or_return_value!(
            ((id as usize) < self.m_num_faults) && (percentage <= 100),
            err,
            err = Err(ErrorCode::KErrInvalid)
        );
     ...
     * @return      KErrInvalid if the inputs are not valid.
     */
    pub fn fail_at_fault_with_lock(
        &mut self,
        id: Identifier,
        num_calls_to_skip: u32,
        num_calls_to_fail: u32,
        take_mutex: bool,
    ) -> FaultInjectionResult {
        let mut err: FaultInjectionResult = Ok(());

        verify_or_return_value!(
            ((id as usize) < self.m_num_faults)
                && (num_calls_to_skip <= u16::MAX.into())
                && (num_calls_to_fail <= u16::MAX.into()),
            err,
            err = Err(ErrorCode::KErrInvalid)
        );
    ...
    pub fn fail_at_fault(
        &mut self,
        id: Identifier,
        num_calls_to_skip: u32,
        num_calls_to_fail: u32,
    ) -> FaultInjectionResult {
        return self.fail_at_fault_with_lock(
            id,
            num_calls_to_skip,
            num_calls_to_fail,
            K_MUTEXT_TAKE,
    ...
    pub fn store_args_at_fault(
        ...
        let mut err: FaultInjectionResult = Ok(());
        verify_or_return_value!(
            ((id as usize) < self.m_num_faults) && args.len() <= u8::MAX.into(),
            err,
            err = Err(ErrorCode::KErrInvalid)
        );
     ...
     * @return      KErrInvalid if the inputs are not valid.
     */
    pub fn remove_callback_at_fault_with_lock(
        &mut self,
        id: Identifier,
        ...
        let mut err: FaultInjectionResult = Ok(());
        verify_or_return_value!(
            ((id as usize) < self.m_num_faults) && callback.is_null() == false,
            err,
            err = Err(ErrorCode::KErrInvalid)
        );
    ...
    pub fn remove_callback_at_fault(
        &mut self,
        id: Identifier,
        callback: *mut Callback,
    ) -> FaultInjectionResult {
     ...
     * @return      KErrInvalid if the inputs are not valid.
     */
    pub fn insert_callback_at_fault(
        &mut self,
        id: Identifier,
        callback: *mut Callback,
    ) -> FaultInjectionResult {
        ...
        while cb.is_null() == false {
            unsafe {
                ...
                if true
                    == ((*cb).m_call_back_fn)(
                        id,
                        ptr::addr_of_mut!(self.m_fault_records[record_index]),
                        (*cb).m_context,
                    )
                {
                    ret_val = true;
     ...
     */
    pub fn check_fault_with_out_args_with_lock(
        &mut self,
        id: Identifier,
        take_mutex: bool,
    ) -> (&[i32], bool) {
    ...
    mod init {
        ...
        fn init_successfully() {
            ...
            unsafe {
                assert_eq!(
                    true,
                    m.init(NUM_FAULTS, &mut FAULT_RECORDS, &MANAGER_NAME, &FAULT_NAMES)
                        .is_ok()
                );
            }
        }
        ...
        fn init_with_0_num_fault() {
            ...
            unsafe {
                assert_eq!(
                    true,
                    m.init(0, &mut FAULT_RECORDS, &MANAGER_NAME, &FAULT_NAMES)
                        .is_err()
                );
            }
        }
        ...
        fn init_with_0_record() {
            ...
            unsafe {
                assert_eq!(
                    true,
                    m.init(NUM_FAULTS, &mut EMPTY_RECORDS, &MANAGER_NAME, &FAULT_NAMES)
                        .is_err()
                );
            }
        }
        ...
        fn init_empty_manager_name() {
            ...
            unsafe {
                assert_eq!(
                    true,
                    m.init(NUM_FAULTS, &mut FAULT_RECORDS, "", &FAULT_NAMES)
                        .is_err()
                );
            }
        }
        ...
        fn init_empty_fault_names() {
            ...
            unsafe {
                assert_eq!(
                    true,
                    m.init(NUM_FAULTS, &mut FAULT_RECORDS, &MANAGER_NAME, &[])
                        .is_err()
                );
            }
        }
    ...
    mod failt_at {
        ...
        fn set_up() {
            unsafe {
                ...
                let _ =
                    FAULT_MANAGER.init(NUM_FAULTS, &mut FAULT_RECORDS, &MANAGER_NAME, &FAULT_NAMES);
            }
        }
        ...
        fn fail_at_randomly_with_id_over_range() {
            ...
            unsafe {
                assert_eq!(
                    true,
                    FAULT_MANAGER
                        .fail_randomly_at_fault(NUM_FAULTS.try_into().unwrap(), 0)
                        .is_err()
                );
            }
        }
        ...
        fn fail_at_fault_successfully() {
            ...
            unsafe {
                assert_eq!(
                    true,
                    FAULT_MANAGER
                        .fail_at_fault_with_lock(0, 0, 0, false)
                        .is_ok()
                );
            }
        }
        ...
        fn fail_at_fault_with_id_over_range() {
            ...
            unsafe {
                assert_eq!(
                    true,
                    FAULT_MANAGER
                        .fail_at_fault_with_lock(NUM_FAULTS.try_into().unwrap(), 0, 0, false)
                        .is_err()
                );
            }
        }
        ...
        fn fail_at_fault_but_num_skip_too_big() {
            ...
            unsafe {
                assert_eq!(
                    true,
                    FAULT_MANAGER
                        .fail_at_fault_with_lock(0, u16::MAX as u32 + 1, 0, false)
                        .is_err()
                );
            }
        }
        ...
        fn fail_at_fault_but_num_check_too_big() {
            ...
            unsafe {
                assert_eq!(
                    true,
                    FAULT_MANAGER
                        .fail_at_fault_with_lock(0, 0, u16::MAX as u32 + 1, false)
                        .is_err()
                );
            }
        }
        ...
        fn reboot_at_fault_with_id_over_range() {
            ...
            unsafe {
                assert_eq!(
                    true,
                    FAULT_MANAGER
                        .reboot_at_fault(NUM_FAULTS.try_into().unwrap())
                        .is_err()
                );
            }
        }
        ...
        fn store_args_at_fault_successfully() {
            ...
            unsafe {
                ...
                assert_eq!(
                    true,
                    FAULT_MANAGER.store_args_at_fault(0, &DATA[0..2]).is_ok()
                );
            }
        }
        ...
        fn store_args_at_fault_with_id_over_range() {
            ...
            unsafe {
                ...
                assert_eq!(
                    true,
                    FAULT_MANAGER
                        .store_args_at_fault(NUM_FAULTS.try_into().unwrap(), &DATA[0..2])
                        .is_err()
                );
            }
        }
        ...
        fn insert_one_successfully() {
            ...
            unsafe {
                assert_eq!(
                    true,
                    FAULT_MANAGER
                        .insert_callback_at_fault(0, ptr::addr_of_mut!(CALLBACK_STUB_1))
                        .is_ok()
                );
                assert_eq!(
                    ptr::addr_of_mut!(CALLBACK_STUB_1),
                    FAULT_MANAGER.get_fault_records()[0].m_callback_list
                );
            }
        }
        ...
        fn insert_with_id_overrange() {
            ...
            unsafe {
                assert_eq!(
                    true,
                    FAULT_MANAGER
                        .insert_callback_at_fault(
                            NUM_FAULTS.try_into().unwrap(),
                            ptr::addr_of_mut!(CALLBACK_STUB_1)
                        )
                        .is_err()
                );
        ...
        fn insert_with_empty_callback() {
            ...
            unsafe {
                assert_eq!(
                    true,
                    FAULT_MANAGER
                        .insert_callback_at_fault(0, ptr::null_mut())
                        .is_err()
                );
            }
        }
        ...
        fn insert_two_successfully() {
            ...
            unsafe {
                ...
                assert_eq!(
                    true,
                    FAULT_MANAGER
                        .insert_callback_at_fault(0, ptr::addr_of_mut!(CALLBACK_STUB_2))
                        .is_ok()
                );
                assert_eq!(
                    ptr::addr_of_mut!(CALLBACK_STUB_2),
                    FAULT_MANAGER.get_fault_records()[0].m_callback_list
                );
            }
        }
        ...
        fn insert_one_remove_one_successfully() {
            ...
            unsafe {
                ...
                assert_eq!(
                    true,
                    FAULT_MANAGER
                        .remove_callback_at_fault(0, ptr::addr_of_mut!(CALLBACK_STUB_1))
                        .is_ok()
                );
                assert_eq!(
                    ptr::addr_of_mut!(S_RANDOM_CB),
                    FAULT_MANAGER.get_fault_records()[0].m_callback_list
                );
            }
        }
    ...
    mod check_fault {
        ...
        fn check_true() {
            ...
            unsafe {
                assert_eq!(
                    true,
                    FAULT_MANAGER
                        .insert_callback_at_fault(0, ptr::addr_of_mut!(CALLBACK_STUB_TRUE))
                        .is_ok()
                );
        ...
        fn check_with_id_overrange() {
            ...
            unsafe {
                ...
                assert_eq!(
                    false,
                    FAULT_MANAGER.check_fault(NUM_FAULTS.try_into().unwrap())
                );
            }
        }
        ...
        fn check_with_false_callback() {
            ...
            unsafe {
                assert_eq!(
                    true,
                    FAULT_MANAGER
                        .insert_callback_at_fault(0, ptr::addr_of_mut!(CALLBACK_STUB_FALSE))
                        .is_ok()
                );
                assert_eq!(false, FAULT_MANAGER.check_fault(0));
        ...
        fn reset_configurations_with_id_overrange() {
            ...
            unsafe {
                ...
                assert_eq!(
                    true,
                    FAULT_MANAGER
                        .reset_configurations(NUM_FAULTS.try_into().unwrap())
                        .is_err()
                );
                assert_eq!(
                    ptr::addr_of_mut!(CALLBACK_STUB_TRUE),
                    FAULT_MANAGER.get_fault_records()[0].m_callback_list
                );
            }
        }
