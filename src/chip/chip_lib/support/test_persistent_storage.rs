...
impl TestPersistentStorage {
    fn sync_set_key_value_raw_internal(&mut self, key_string: &str, buffer: * const u8, size: usize) -> ChipErrorResult {
    ...
    fn sync_get_key_value_raw_internal(&self, key_string: &str, buffer: * mut u8, size: &mut usize) -> ChipErrorResult {
...
impl PersistentStorageDelegate for TestPersistentStorage {
    ...
    fn sync_get_key_value(&self, key: &str, buffer: &mut [u8]) -> Result<usize, ChipError> {
        ...
        self.sync_get_key_value_raw(key, buffer.as_mut_ptr(), &mut size).map_err(|e| e)?;
